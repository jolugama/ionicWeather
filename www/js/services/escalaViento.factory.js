(function() {
  ...
  function escalaVientoService() {
    ...
    var service = {
      getDescripcion: _getDescripcion,
      vientoJson:_vientoJson
    };

    return service;

    function _vientoJson(){
      return {
        "operation": true,
        "results": [
          {
    ...
    function _getDescripcion(metros){
      var resultado=['',''];
      for (var i =0; i<_vientoJson().results.length ; i++) {
        var arrayMetros=_vientoJson().results[i].ms.split('-');
        if(metros>=parseFloat(arrayMetros[0]) && metros<=parseFloat(arrayMetros[1])){
          resultado=[_vientoJson().results[i].estado,_vientoJson().results[i].desc];
          return resultado;
        }
      }
    }
